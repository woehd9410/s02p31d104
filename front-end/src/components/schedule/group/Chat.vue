...
export default {
  ...
  methods: {
    findAllChattings: async function() {
      await axios
        .get(`${process.env.VUE_APP_CHAT_SERVER}/chat/chattings/${this.roomId}`)
        .then((response) => {
          if (response.data != "") {
    ...
    connect: async function() {
      this.socket = new SockJS(`${process.env.VUE_APP_CHAT_SERVER}//ws-stomp`);
    ...
    recvMessage: async function(recv) {
      if(recv.type == "ENTER") this.$emit("inChat",recv.email)
      console.log(recv);
      var temp = {
        type: recv.type,
